...
//	The purpose of the NullAudio sample is to provide a bare bones
//implementations to 	illustrate the minimal set of things a driver has to do.
//The sample driver has the following 	qualities:
//	- a plug-in
//		- custom property with the selector kPlugIn_CustomPropertyID =
//'PCst'
//	- a box
//	- a device
...
//		- all are for illustration purposes only and do not actually
//manipulate data

//	Declare the internal object ID numbers for all the objects this driver
//implements. Note that 	this driver has a fixed set of objects that never grows
//or shrinks. If this were not the case, 	the driver would need to have a means
//to dynamically allocate these IDs. It's important to 	realize that a lot of the
//structure of this driver is vastly simpler when the IDs are all 	known a
//priori. Comments in the code will try to identify some of these
//simplifications and 	point out what a more complicated driver will need to do.
enum {
...
//	Declare the stuff that tracks the state of the plug-in, the device and
//its sub-objects. 	Note that we use global variables here because this driver
//only ever has a single device. If 	multiple devices were supported, this state
//would need to be encapsulated in one or more structs 	so that each object's
//state can be tracked individually. 	Note also that we share a single mutex
//across all objects to be thread safe for the same reason.
...
static HRESULT NullAudio_QueryInterface(void* inDriver,
                                        REFIID inUUID,
                                        LPVOID* outInterface);
...
static OSStatus NullAudio_AddDeviceClient(
    AudioServerPlugInDriverRef inDriver,
    AudioObjectID inDeviceObjectID,
    const AudioServerPlugInClientInfo* inClientInfo);
static OSStatus NullAudio_RemoveDeviceClient(
    AudioServerPlugInDriverRef inDriver,
    AudioObjectID inDeviceObjectID,
    const AudioServerPlugInClientInfo* inClientInfo);
...
static OSStatus NullAudio_BeginIOOperation(
    AudioServerPlugInDriverRef inDriver,
    AudioObjectID inDeviceObjectID,
    UInt32 inClientID,
    UInt32 inOperationID,
...
static OSStatus NullAudio_DoIOOperation(
    AudioServerPlugInDriverRef inDriver,
    AudioObjectID inDeviceObjectID,
...
static OSStatus NullAudio_EndIOOperation(
    AudioServerPlugInDriverRef inDriver,
    AudioObjectID inDeviceObjectID,
    UInt32 inClientID,
    UInt32 inOperationID,
...
//	Implementation
static Boolean NullAudio_HasPlugInProperty(
    AudioServerPlugInDriverRef inDriver,
    AudioObjectID inObjectID,
    pid_t inClientProcessID,
    const AudioObjectPropertyAddress* inAddress);
...
static Boolean NullAudio_HasBoxProperty(
    AudioServerPlugInDriverRef inDriver,
    AudioObjectID inObjectID,
    pid_t inClientProcessID,
    const AudioObjectPropertyAddress* inAddress);
...
static Boolean NullAudio_HasDeviceProperty(
    AudioServerPlugInDriverRef inDriver,
    AudioObjectID inObjectID,
    pid_t inClientProcessID,
    const AudioObjectPropertyAddress* inAddress);
...
static Boolean NullAudio_HasStreamProperty(
    AudioServerPlugInDriverRef inDriver,
    AudioObjectID inObjectID,
    pid_t inClientProcessID,
    const AudioObjectPropertyAddress* inAddress);
...
static Boolean NullAudio_HasControlProperty(
    AudioServerPlugInDriverRef inDriver,
    AudioObjectID inObjectID,
    pid_t inClientProcessID,
    const AudioObjectPropertyAddress* inAddress);
...
void* NullAudio_Create(CFAllocatorRef inAllocator,
  ...
  //static iniitalization when the bundle is loaded, all that 	needs to be done
  ...
  //initilization should be handled in the Initialize() method of 	the driver's
...
static OSStatus NullAudio_Initialize(AudioServerPlugInDriverRef inDriver,
  ...
  //as the device list) to get the inital set of objects the driver is
  //	publishing. So, there is no need to notifiy the HAL about any objects
  ...
  Float64 theHostClockFrequency =
      (Float64)theTimeBaseInfo.numer / (Float64)theTimeBaseInfo.denom;
...
static OSStatus NullAudio_PerformDeviceConfigurationChange(
  ...
  //configuation change that it 	had requested via a call to the host method,
  ...
  //device that isn't a control. For this 	change, the new sample rate is passed
  //in the inChangeAction argument.
...
static Boolean NullAudio_HasProperty(
  ...
  //detailed commentary about each 	property in the NullAudio_GetPropertyData()
  //method.
  switch (inObjectID) {
    case kObjectID_PlugIn:
      theAnswer = NullAudio_HasPlugInProperty(inDriver, inObjectID,
                                              inClientProcessID, inAddress);
      break;

    case kObjectID_Box:
      theAnswer = NullAudio_HasBoxProperty(inDriver, inObjectID,
                                           inClientProcessID, inAddress);
      break;

    case kObjectID_Device:
      theAnswer = NullAudio_HasDeviceProperty(inDriver, inObjectID,
                                              inClientProcessID, inAddress);
      break;

    case kObjectID_Stream_Input:
    case kObjectID_Stream_Output:
      theAnswer = NullAudio_HasStreamProperty(inDriver, inObjectID,
                                              inClientProcessID, inAddress);
      break;
    ...
    case kObjectID_DataDestination_PlayThru_Master:
      theAnswer = NullAudio_HasControlProperty(inDriver, inObjectID,
                                               inClientProcessID, inAddress);
      break;
  };
...
static OSStatus NullAudio_IsPropertySettable(
  ...
  FailWithAction(
      ...
      Done, "NullAudio_IsPropertySettable: no place to put the return value");
...
static OSStatus NullAudio_GetPropertyDataSize(
  ...
  FailWithAction(
      ...
      Done, "NullAudio_GetPropertyDataSize: no place to put the return value");
...
static OSStatus NullAudio_GetPropertyData(
  ...
  FailWithAction(
      outDataSize == NULL, theAnswer = kAudioHardwareIllegalOperationError,
      Done, "NullAudio_GetPropertyData: no place to put the return value size");
  ...
  switch (inObjectID) {
    case kObjectID_PlugIn:
      theAnswer = NullAudio_GetPlugInPropertyData(
          inDriver, inObjectID, inClientProcessID, inAddress,
          inQualifierDataSize, inQualifierData, inDataSize, outDataSize,
          outData);
      break;
...
#pragma unused(inClientProcessID)
  ...
  //NullAudio_GetPlugInPropertyData() method.
  ...
  switch (inAddress->mSelector) {
    ...
    case kPlugIn_CustomPropertyID:
      FailWithAction(inQualifierDataSize != sizeof(CFStringRef),
                     ...
                     "NullAudio_GetPlugInPropertyDataSize: the qualifier is "
                     "the wrong size for kPlugIn_CustomPropertyID");
      FailWithAction(inQualifierData == NULL,
                     theAnswer = kAudioHardwareBadPropertySizeError, Done,
                     "NullAudio_GetPlugInPropertyDataSize: no qualifier for "
                     "kPlugIn_CustomPropertyID");
      DebugMsg(
          "NullAudio_GetPlugInPropertyDataSize: the qualifier passed to us "
          "was:");
  ...
  switch (inAddress->mSelector) {
    case kAudioObjectPropertyBaseClass:
      ...
      FailWithAction(
          ...
          theAnswer = kAudioHardwareBadPropertySizeError, Done,
          "NullAudio_GetPlugInPropertyData: not enough space for the return "
    ...
    case kAudioObjectPropertyClass:
      ...
      FailWithAction(
          ...
          theAnswer = kAudioHardwareBadPropertySizeError, Done,
          "NullAudio_GetPlugInPropertyData: not enough space for the return "
          "value of kAudioObjectPropertyClass for the plug-in");
    ...
    case kAudioObjectPropertyOwner:
      ...
      FailWithAction(
          ...
          theAnswer = kAudioHardwareBadPropertySizeError, Done,
          "NullAudio_GetPlugInPropertyData: not enough space for the return "
          "value of kAudioObjectPropertyOwner for the plug-in");
    ...
    case kAudioObjectPropertyOwnedObjects:
      //	Calculate the number of items that have been requested. Note
      //that this 	number is allowed to be smaller than the actual size of the
      //list. In such 	case, only that number of items will be returned
      theNumberItemsToFetch = inDataSize / sizeof(AudioObjectID);

      //	Clamp that to the number of boxes this driver implements (which
      //is just 1)
    ...
    case kAudioPlugInPropertyTranslateUIDToBox:
      //	This property takes the CFString passed in the qualifier and
      //converts that 	to the object ID of the box it corresponds to. For this
      //driver, there is 	just the one box. Note that it is not an error if the
      //string in the 	qualifier doesn't match any devices. In such case,
      //kAudioObjectUnknown is 	the object ID to return.
      FailWithAction(
          inDataSize < sizeof(AudioObjectID),
          theAnswer = kAudioHardwareBadPropertySizeError, Done,
          "NullAudio_GetPlugInPropertyData: not enough space for the return "
          "value of kAudioPlugInPropertyTranslateUIDToBox");
      FailWithAction(inQualifierDataSize != sizeof(CFStringRef),
                     theAnswer = kAudioHardwareBadPropertySizeError, Done,
                     "NullAudio_GetPlugInPropertyData: the qualifier is the "
                     "wrong size for kAudioPlugInPropertyTranslateUIDToBox");
      FailWithAction(inQualifierData == NULL,
                     theAnswer = kAudioHardwareBadPropertySizeError, Done,
                     "NullAudio_GetPlugInPropertyData: no qualifier for "
                     "kAudioPlugInPropertyTranslateUIDToBox");
      if (CFStringCompare(*((CFStringRef*)inQualifierData), CFSTR(kBox_UID),
                          0) == kCFCompareEqualTo) {
    ...
    case kAudioPlugInPropertyDeviceList:
      ...
      //	Clamp that to the number of devices this driver implements
      //(which is just 1 if the 	box has been acquired)
    ...
    case kAudioPlugInPropertyTranslateUIDToDevice:
      //	This property takes the CFString passed in the qualifier and
      //converts that 	to the object ID of the device it corresponds to. For this
      //driver, there is 	just the one device. Note that it is not an error if
      //the string in the 	qualifier doesn't match any devices. In such case,
      //kAudioObjectUnknown is 	the object ID to return.
      FailWithAction(
          inDataSize < sizeof(AudioObjectID),
          theAnswer = kAudioHardwareBadPropertySizeError, Done,
          "NullAudio_GetPlugInPropertyData: not enough space for the return "
          "value of kAudioPlugInPropertyTranslateUIDToDevice");
      FailWithAction(inQualifierDataSize != sizeof(CFStringRef),
                     theAnswer = kAudioHardwareBadPropertySizeError, Done,
                     "NullAudio_GetPlugInPropertyData: the qualifier is the "
      ...
      if (CFStringCompare(*((CFStringRef*)inQualifierData), CFSTR(kDevice_UID),
                          0) == kCFCompareEqualTo) {
    ...
    case kAudioPlugInPropertyResourceBundle:
      //	The resource bundle is a path relative to the path of the
      //plug-in's bundle. 	To specify that the plug-in bundle itself should be
      //used, we just return the 	empty string.
      FailWithAction(inDataSize < sizeof(AudioObjectID),
                     theAnswer = kAudioHardwareBadPropertySizeError, Done,
                     "NullAudio_GetPlugInPropertyData: not enough space for "
                     "the return value of kAudioPlugInPropertyResourceBundle");
    ...
    case kAudioObjectPropertyCustomPropertyInfoList:
      ...
      //AudioServerPlugInCustomPropertyInfo's that 	describe the type of data
      //used by any custom properties. For this example, 	the plug-in supports a
      //single property whose data type is a CFString and 	whose qualifier is a
      //CFString.
      FailWithAction(
          inDataSize < sizeof(AudioServerPlugInCustomPropertyInfo),
          theAnswer = kAudioHardwareBadPropertySizeError, Done,
          "NullAudio_GetPlugInPropertyData: not enough space for the return "
          "value of kAudioObjectPropertyCustomPropertyInfoList");
    ...
    case kPlugIn_CustomPropertyID:
      FailWithAction(inDataSize < sizeof(CFStringRef),
                     theAnswer = kAudioHardwareBadPropertySizeError, Done,
                     "NullAudio_GetPlugInPropertyData: not enough space for "
                     "the return value of kPlugIn_CustomPropertyID");
      FailWithAction(inQualifierDataSize != sizeof(CFPropertyListRef),
                     theAnswer = kAudioHardwareBadPropertySizeError, Done,
                     "NullAudio_GetPlugInPropertyData: the qualifier is the "
                     "wrong size for kPlugIn_CustomPropertyID");
      FailWithAction(inQualifierData == NULL,
                     theAnswer = kAudioHardwareBadPropertySizeError, Done,
                     "NullAudio_GetPlugInPropertyData: no qualifier for "
                     "kPlugIn_CustomPropertyID");
...
#pragma unused(inClientProcessID, inQualifierDataSize, inQualifierData, \
  ...
  FailWithAction(outNumberPropertiesChanged == NULL,
                 theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "NullAudio_SetPlugInPropertyData: no place to return the "
  ...
  *outNumberPropertiesChanged = 0;
  ...
  switch (inAddress->mSelector) {
    case kPlugIn_CustomPropertyID:
      FailWithAction(inDataSize != sizeof(CFStringRef),
                     theAnswer = kAudioHardwareBadPropertySizeError, Done,
                     "NullAudio_SetPlugInPropertyData: not enough space for "
                     "the return value of kPlugIn_CustomPropertyID");
      FailWithAction(inQualifierDataSize != sizeof(CFPropertyListRef),
                     theAnswer = kAudioHardwareBadPropertySizeError, Done,
                     "NullAudio_SetPlugInPropertyData: the qualifier is the "
                     "wrong size for kPlugIn_CustomPropertyID");
      FailWithAction(inQualifierData == NULL,
                     theAnswer = kAudioHardwareBadPropertySizeError, Done,
                     "NullAudio_SetPlugInPropertyData: no qualifier for "
                     "kPlugIn_CustomPropertyID");
      DebugMsg(
...
#pragma unused(inClientProcessID)
  ...
  //properties plus a few 	extras that are useful but not required. There is more
  //detailed commentary about each 	property in the
  //NullAudio_GetBoxPropertyData() method.
  switch (inAddress->mSelector) {
    case kAudioObjectPropertyBaseClass:
    case kAudioObjectPropertyClass:
    case kAudioObjectPropertyOwner:
  ...
  switch (inAddress->mSelector) {
    case kAudioObjectPropertyBaseClass:
      ...
      FailWithAction(
          inDataSize < sizeof(AudioClassID),
          theAnswer = kAudioHardwareBadPropertySizeError, Done,
          "NullAudio_GetBoxPropertyData: not enough space for the return value "
          "of kAudioObjectPropertyBaseClass for the box");
      *((AudioClassID*)outData) = kAudioObjectClassID;
      *outDataSize = sizeof(AudioClassID);
      break;
    ...
    case kAudioObjectPropertyClass:
      ...
      FailWithAction(inDataSize < sizeof(AudioClassID),
                     theAnswer = kAudioHardwareBadPropertySizeError, Done,
                     "NullAudio_GetBoxPropertyData: not enough space for the "
    ...
    case kAudioObjectPropertyOwnedObjects:
      //	This returns the objects directly owned by the object. Boxes
      //don't own anything.
      *outDataSize = 0;
      break;

    case kAudioObjectPropertyIdentify:
      //	This is used to highling the device in the UI, but it's value
      //has no meaning
      FailWithAction(
          inDataSize < sizeof(UInt32),
          theAnswer = kAudioHardwareBadPropertySizeError, Done,
          "NullAudio_GetBoxPropertyData: not enough space for the return value "
          "of kAudioObjectPropertyIdentify for the box");
      *((UInt32*)outData) = 0;
      *outDataSize = sizeof(UInt32);
      break;
    ...
    case kAudioBoxPropertyTransportType:
      //	This value represents how the device is attached to the system.
      //This can be 	any 32 bit integer, but common values for this property are
    ...
    case kAudioBoxPropertyIsProtected:
      //	Indicates whether or not the box has requires authentication to
      //use
      FailWithAction(
          inDataSize < sizeof(UInt32),
          theAnswer = kAudioHardwareBadPropertySizeError, Done,
          "NullAudio_GetBoxPropertyData: not enough space for the return value "
          "of kAudioBoxPropertyIsProtected for the box");
      *((UInt32*)outData) = 0;
      *outDataSize = sizeof(UInt32);
      break;

    case kAudioBoxPropertyAcquired:
      //	When set to a non-zero value, the device is acquired for use by
      //the local machine
      FailWithAction(inDataSize < sizeof(UInt32),
                     theAnswer = kAudioHardwareBadPropertySizeError, Done,
                     "NullAudio_GetBoxPropertyData: not enough space for the "
    ...
    case kAudioBoxPropertyAcquisitionFailed:
      //	This is used for notifications to say when an attempt to acquire
      //a device has failed.
      FailWithAction(
          inDataSize < sizeof(UInt32),
          theAnswer = kAudioHardwareBadPropertySizeError, Done,
    ...
    case kAudioBoxPropertyDeviceList:
      ...
      if (gBox_Acquired) {
        FailWithAction(
            inDataSize < sizeof(AudioObjectID),
            theAnswer = kAudioHardwareBadPropertySizeError, Done,
            "NullAudio_GetBoxPropertyData: not enough space for the return "
            "value of kAudioBoxPropertyDeviceList for the box");
        *((AudioObjectID*)outData) = kObjectID_Device;
        *outDataSize = sizeof(AudioObjectID);
      } else {
        *outDataSize = 0;
...
#pragma unused(inClientProcessID, inQualifierDataSize, inQualifierData, \
  ...
  FailWithAction(outNumberPropertiesChanged == NULL,
                 theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "NullAudio_SetBoxPropertyData: no place to return the number "
                 "of properties that changed");
  FailWithAction(outChangedAddresses == NULL,
                 theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "NullAudio_SetBoxPropertyData: no place to return the "
                 "properties that changed");
  FailWithAction(inObjectID != kObjectID_Box,
                 theAnswer = kAudioHardwareBadObjectError, Done,
  ...
  switch (inAddress->mSelector) {
    case kAudioObjectPropertyName:
      //	Boxes should allow their name to be editable
      {
        FailWithAction(inDataSize != sizeof(CFStringRef),
                       theAnswer = kAudioHardwareBadPropertySizeError, Done,
                       "NullAudio_SetBoxPropertyData: wrong size for the data "
                       "for kAudioObjectPropertyName");
        FailWithAction(inData == NULL,
                       theAnswer = kAudioHardwareIllegalOperationError, Done,
                       "NullAudio_SetBoxPropertyData: no data to set for "
    ...
    case kAudioObjectPropertyIdentify:
      //	since we don't have any actual hardware to flash, we will
      //schedule a notificaiton for 	this property off into the future as a
      //testing thing. Note that a real implementation 	of this property should
      //only send the notificaiton if the hardware wants the app to 	flash it's
      //UI for the device.
      {
        syslog(LOG_NOTICE,
               "The identify property has been set on the Box implemented by "
               "the NullAudio driver.");
        FailWithAction(inDataSize != sizeof(UInt32),
                       theAnswer = kAudioHardwareBadPropertySizeError, Done,
                       "NullAudio_SetBoxPropertyData: wrong size for the data "
    ...
    case kAudioBoxPropertyAcquired:
      //	When the box is acquired, it means the contents, namely the
      //device, are available to the system
      {
        FailWithAction(inDataSize != sizeof(UInt32),
                       theAnswer = kAudioHardwareBadPropertySizeError, Done,
                       "NullAudio_SetBoxPropertyData: wrong size for the data "
          ...
          dispatch_async(
              ...
              ^() {
                AudioObjectPropertyAddress theAddress = {
                    ...
                    kAudioObjectPropertyElementMain};
                gPlugIn_Host->PropertiesChanged(gPlugIn_Host, kObjectID_PlugIn,
                                                1, &theAddress);
              });
        }
        pthread_mutex_unlock(&gPlugIn_StateMutex);
...
#pragma unused(inClientProcessID)
  ...
  //properties plus a few 	extras that are useful but not required. There is more
  //detailed commentary about each 	property in the
  //NullAudio_GetDevicePropertyData() method.
  switch (inAddress->mSelector) {
    case kAudioObjectPropertyBaseClass:
    case kAudioObjectPropertyClass:
    case kAudioObjectPropertyOwner:
  ...
  switch (inAddress->mSelector) {
    case kAudioObjectPropertyBaseClass:
      ...
      FailWithAction(
          inDataSize < sizeof(AudioClassID),
          theAnswer = kAudioHardwareBadPropertySizeError, Done,
          "NullAudio_GetDevicePropertyData: not enough space for the return "
    ...
    case kAudioObjectPropertyClass:
      //	The class is always kAudioDeviceClassID for devices created by
      //drivers
      FailWithAction(
          inDataSize < sizeof(AudioClassID),
          theAnswer = kAudioHardwareBadPropertySizeError, Done,
          "NullAudio_GetDevicePropertyData: not enough space for the return "
          "value of kAudioObjectPropertyClass for the device");
    ...
    case kAudioObjectPropertyOwner:
      ...
      FailWithAction(
          ...
          theAnswer = kAudioHardwareBadPropertySizeError, Done,
          "NullAudio_GetDevicePropertyData: not enough space for the return "
          "value of kAudioObjectPropertyOwner for the device");
      *((AudioObjectID*)outData) = kObjectID_PlugIn;
      *outDataSize = sizeof(AudioObjectID);
      break;
    ...
    case kAudioObjectPropertyName:
      ...
      FailWithAction(
          ...
          theAnswer = kAudioHardwareBadPropertySizeError, Done,
          "NullAudio_GetDevicePropertyData: not enough space for the return "
          "value of kAudioObjectPropertyName for the device");
    ...
    case kAudioObjectPropertyElementName:
      ...
      FailWithAction(
          ...
          theAnswer = kAudioHardwareBadPropertySizeError, Done,
          "NullAudio_GetDevicePropertyData: not enough space for the return "
          "value of kAudioObjectPropertyElementName for the device");
    ...
    case kAudioObjectPropertyOwnedObjects:
      ...
      //	The device owns its streams and controls. Note that what is
      //returned here 	depends on the scope requested.
      switch (inAddress->mScope) {
        case kAudioObjectPropertyScopeGlobal:
          ...
          //	fill out the list with as many objects as requested, which is
          //everything
    ...
    case kAudioDevicePropertyDeviceUID:
      //	This is a CFString that is a persistent token that can identify
      //the same 	audio device across boot sessions. Note that two instances of
      //the same 	device must have different values for this property.
      FailWithAction(
          inDataSize < sizeof(CFStringRef),
          theAnswer = kAudioHardwareBadPropertySizeError, Done,
          "NullAudio_GetDevicePropertyData: not enough space for the return "
    ...
    case kAudioDevicePropertyModelUID:
      //	This is a CFString that is a persistent token that can identify
      //audio 	devices that are the same kind of device. Note that two instances
      //of the 	save device must have the same value for this property.
      FailWithAction(
          inDataSize < sizeof(CFStringRef),
          theAnswer = kAudioHardwareBadPropertySizeError, Done,
          "NullAudio_GetDevicePropertyData: not enough space for the return "
    ...
    case kAudioDevicePropertyRelatedDevices:
      //	The related devices property identifys device objects that are
      //very closely 	related. Generally, this is for relating devices that are
      //packaged together 	in the hardware such as when the input side and the
      //output side of a piece 	of hardware can be clocked separately and
      //therefore need to be represented 	as separate AudioDevice objects. In
      //such case, both devices would report 	that they are related to each
      //other. Note that at minimum, a device is 	related to itself, so this list
      //will always be at least one item long.
    ...
    case kAudioDevicePropertyClockDomain:
      //	This property allows the device to declare what other devices it
      //is 	synchronized with in hardware. The way it works is that if two
      //devices have 	the same value for this property and the value is not zero,
      //then the two 	devices are synchronized in hardware. Note that a device
      //that either can't 	be synchronized with others or doesn't know should
      //return 0 for this 	property.
      FailWithAction(
          inDataSize < sizeof(UInt32),
          theAnswer = kAudioHardwareBadPropertySizeError, Done,
          "NullAudio_GetDevicePropertyData: not enough space for the return "
    ...
    case kAudioDevicePropertyDeviceIsAlive:
      //	This property returns whether or not the device is alive. Note
      //that it is 	note uncommon for a device to be dead but still momentarily
      //availble in the 	device list. In the case of this device, it will always
      //be alive.
      FailWithAction(
          inDataSize < sizeof(UInt32),
          theAnswer = kAudioHardwareBadPropertySizeError, Done,
    ...
    case kAudioDevicePropertyDeviceIsRunning:
      //	This property returns whether or not IO is running for the
      //device. Note that 	we need to take both the state lock to check this
      //value for thread safety.
      FailWithAction(
          inDataSize < sizeof(UInt32),
          theAnswer = kAudioHardwareBadPropertySizeError, Done,
          "NullAudio_GetDevicePropertyData: not enough space for the return "
    ...
    case kAudioDevicePropertyDeviceCanBeDefaultDevice:
      //	This property returns whether or not the device wants to be able
      //to be the 	default device for content. This is the device that iTunes and
      //QuickTime 	will use to play their content on and FaceTime will use as
      //it's microhphone. 	Nearly all devices should allow for this.
      FailWithAction(
          inDataSize < sizeof(UInt32),
          theAnswer = kAudioHardwareBadPropertySizeError, Done,
          "NullAudio_GetDevicePropertyData: not enough space for the return "
          "value of kAudioDevicePropertyDeviceCanBeDefaultDevice for the "
          "device");
      *((UInt32*)outData) = 1;
      *outDataSize = sizeof(UInt32);
      break;
    ...
    case kAudioDevicePropertyDeviceCanBeDefaultSystemDevice:
      //	This property returns whether or not the device wants to be the
      //system 	default device. This is the device that is used to play interface
      //sounds and 	other incidental or UI-related sounds on. Most devices should
      //allow this 	although devices with lots of latency may not want to.
      FailWithAction(
          inDataSize < sizeof(UInt32),
          theAnswer = kAudioHardwareBadPropertySizeError, Done,
          "NullAudio_GetDevicePropertyData: not enough space for the return "
    ...
    case kAudioObjectPropertyControlList:
      ...
      for (theItemIndex = 0; theItemIndex < theNumberItemsToFetch;
        ...
        if (theItemIndex < 3) {
          ((AudioObjectID*)outData)[theItemIndex] =
              kObjectID_Volume_Input_Master + theItemIndex;
        } else {
          ((AudioObjectID*)outData)[theItemIndex] =
    ...
    case kAudioDevicePropertySafetyOffset:
      //	This property returns the how close to now the HAL can read and
      //write. For 	this, device, the value is 0 due to the fact that it always
      //vends silence.
      FailWithAction(
          inDataSize < sizeof(UInt32),
          theAnswer = kAudioHardwareBadPropertySizeError, Done,
    ...
    case kAudioDevicePropertyNominalSampleRate:
      ...
      //Note that we 	only need to take the state lock to get this value.
      FailWithAction(
    ...
    case kAudioDevicePropertyAvailableNominalSampleRates:
      //	This returns all nominal sample rates the device supports as an
      //array of 	AudioValueRangeStructs. Note that for discrete sampler rates,
      //the range 	will have the minimum value equal to the maximum value.
    ...
    case kAudioDevicePropertyPreferredChannelsForStereo:
      //	This property returns which two channesl to use as left/right
      //for stereo 	data by default. Note that the channel numbers are 1-based.xz
      FailWithAction(
    ...
    case kAudioDevicePropertyPreferredChannelLayout:
      //	This property returns the default AudioChannelLayout to use for
      //the device 	by default. For this device, we return a stereo ACL.
      {
        ...
        FailWithAction(
            ...
            theAnswer = kAudioHardwareBadPropertySizeError, Done,
            "NullAudio_GetDevicePropertyData: not enough space for the return "
            "value of kAudioDevicePropertyPreferredChannelLayout for the "
            "device");
    ...
    case kAudioDevicePropertyZeroTimeStampPeriod:
      //	This property returns how many frames the HAL should expect to
      //see between 	successive sample times in the zero time stamps this device
      //provides.
      FailWithAction(
          inDataSize < sizeof(UInt32),
          theAnswer = kAudioHardwareBadPropertySizeError, Done,
    ...
    case kAudioDevicePropertyIcon: {
      //	This is a CFURL that points to the device's Icon in the
      ...
      FailWithAction(theBundle == NULL,
                     ...
                     "NullAudio_GetDevicePropertyData: could not get the "
                     "plug-in bundle for kAudioDevicePropertyIcon");
      ...
      FailWithAction(theURL == NULL, theAnswer = kAudioHardwareUnspecifiedError,
                     Done,
                     "NullAudio_GetDevicePropertyData: could not get the URL "
                     "for kAudioDevicePropertyIcon");
...
#pragma unused(inClientProcessID, inQualifierDataSize, inQualifierData)
  ...
  FailWithAction(outNumberPropertiesChanged == NULL,
                 theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "NullAudio_SetDevicePropertyData: no place to return the "
                 "number of properties that changed");
  FailWithAction(outChangedAddresses == NULL,
                 theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "NullAudio_SetDevicePropertyData: no place to return the "
...
#pragma unused(inClientProcessID)
  ...
  //properties plus a few 	extras that are useful but not required. There is more
  //detailed commentary about each 	property in the
  //NullAudio_GetStreamPropertyData() method.
  switch (inAddress->mSelector) {
    case kAudioObjectPropertyBaseClass:
    case kAudioObjectPropertyClass:
    case kAudioObjectPropertyOwner:
  ...
  switch (inAddress->mSelector) {
    case kAudioObjectPropertyBaseClass:
      ...
      FailWithAction(
          inDataSize < sizeof(AudioClassID),
          theAnswer = kAudioHardwareBadPropertySizeError, Done,
          "NullAudio_GetStreamPropertyData: not enough space for the return "
    ...
    case kAudioObjectPropertyClass:
      //	The class is always kAudioStreamClassID for streams created by
      //drivers
      FailWithAction(
          inDataSize < sizeof(AudioClassID),
          theAnswer = kAudioHardwareBadPropertySizeError, Done,
    ...
    case kAudioObjectPropertyName:
      ...
      FailWithAction(
          ...
          theAnswer = kAudioHardwareBadPropertySizeError, Done,
          "NullAudio_GetStreamPropertyData: not enough space for the return "
          "value of kAudioObjectPropertyName for the stream");
    ...
    case kAudioStreamPropertyIsActive:
      //	This property tells the device whether or not the given stream
      //is going to 	be used for IO. Note that we need to take the state lock to
      //examine this 	value.
      FailWithAction(
          inDataSize < sizeof(UInt32),
          theAnswer = kAudioHardwareBadPropertySizeError, Done,
    ...
    case kAudioStreamPropertyDirection:
      //	This returns whether the stream is an input stream or an output
      //stream.
      FailWithAction(
          inDataSize < sizeof(UInt32),
          theAnswer = kAudioHardwareBadPropertySizeError, Done,
    ...
    case kAudioStreamPropertyTerminalType:
      //	This returns a value that indicates what is at the other end of
      //the stream 	such as a speaker or headphones, or a microphone. Values for
      //this property 	are defined in <CoreAudio/AudioHardwareBase.h>
      FailWithAction(
          inDataSize < sizeof(UInt32),
          theAnswer = kAudioHardwareBadPropertySizeError, Done,
          "NullAudio_GetStreamPropertyData: not enough space for the return "
    ...
    case kAudioStreamPropertyStartingChannel:
      //	This property returns the absolute channel number for the first
      //channel in 	the stream. For exmaple, if a device has two output streams
      //with two 	channels each, then the starting channel number for the first
      //stream is 1 	and ths starting channel number fo the second stream is 3.
      FailWithAction(
          inDataSize < sizeof(UInt32),
          theAnswer = kAudioHardwareBadPropertySizeError, Done,
          "NullAudio_GetStreamPropertyData: not enough space for the return "
    ...
    case kAudioStreamPropertyLatency:
      //	This property returns any additonal presentation latency the
      //stream has.
      FailWithAction(
          inDataSize < sizeof(UInt32),
          theAnswer = kAudioHardwareBadPropertySizeError, Done,
    ...
    case kAudioStreamPropertyPhysicalFormat:
      ...
      //	AudioStreamBasicDescription. Note that we need to hold the state
      //lock to get 	this value. 	Note that for devices that don't override the
      //mix operation, the virtual 	format has to be the same as the physical
      //format.
      FailWithAction(
    ...
    case kAudioStreamPropertyAvailableVirtualFormats:
    case kAudioStreamPropertyAvailablePhysicalFormats:
      //	This returns an array of AudioStreamRangedDescriptions that
      //describe what 	formats are supported.
      ...
      //	fill out the return array
      if (theNumberItemsToFetch > 0) {
        ((AudioStreamRangedDescription*)outData)[0].mFormat.mSampleRate =
            44100.0;
        ((AudioStreamRangedDescription*)outData)[0].mFormat.mFormatID =
            kAudioFormatLinearPCM;
        ...
        ((AudioStreamRangedDescription*)outData)[0].mFormat.mFramesPerPacket =
            1;
        ((AudioStreamRangedDescription*)outData)[0].mFormat.mBytesPerFrame = 8;
        ((AudioStreamRangedDescription*)outData)[0].mFormat.mChannelsPerFrame =
            2;
        ((AudioStreamRangedDescription*)outData)[0].mFormat.mBitsPerChannel =
            32;
        ((AudioStreamRangedDescription*)outData)[0].mSampleRateRange.mMinimum =
            44100.0;
        ((AudioStreamRangedDescription*)outData)[0].mSampleRateRange.mMaximum =
            44100.0;
      }
      if (theNumberItemsToFetch > 1) {
        ((AudioStreamRangedDescription*)outData)[1].mFormat.mSampleRate =
            48000.0;
        ((AudioStreamRangedDescription*)outData)[1].mFormat.mFormatID =
            kAudioFormatLinearPCM;
        ...
        ((AudioStreamRangedDescription*)outData)[1].mFormat.mFramesPerPacket =
            1;
        ((AudioStreamRangedDescription*)outData)[1].mFormat.mBytesPerFrame = 8;
        ((AudioStreamRangedDescription*)outData)[1].mFormat.mChannelsPerFrame =
            2;
        ((AudioStreamRangedDescription*)outData)[1].mFormat.mBitsPerChannel =
            32;
        ((AudioStreamRangedDescription*)outData)[1].mSampleRateRange.mMinimum =
            48000.0;
        ((AudioStreamRangedDescription*)outData)[1].mSampleRateRange.mMaximum =
            48000.0;
      }

      //	report how much we wrote
      *outDataSize =
          theNumberItemsToFetch * sizeof(AudioStreamRangedDescription);
      break;
...
#pragma unused(inClientProcessID, inQualifierDataSize, inQualifierData)
  ...
  FailWithAction(outNumberPropertiesChanged == NULL,
                 theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "NullAudio_SetStreamPropertyData: no place to return the "
                 "number of properties that changed");
  FailWithAction(outChangedAddresses == NULL,
                 theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "NullAudio_SetStreamPropertyData: no place to return the "
  ...
  *outNumberPropertiesChanged = 0;
  ...
  switch (inAddress->mSelector) {
    case kAudioStreamPropertyIsActive:
      //	Changing the active state of a stream doesn't affect IO or
      //change the structure 	so we can just save the state and send the
      //notification.
      FailWithAction(inDataSize != sizeof(UInt32),
                     theAnswer = kAudioHardwareBadPropertySizeError, Done,
                     "NullAudio_SetStreamPropertyData: wrong size for the data "
                     "for kAudioDevicePropertyNominalSampleRate");
      pthread_mutex_lock(&gPlugIn_StateMutex);
    ...
    case kAudioStreamPropertyPhysicalFormat:
      ...
      //	RequestConfigChange/PerformConfigChange machinery. Note that
      //because this 	device only supports 2 channel 32 bit float data, the only
      ...
      FailWithAction(((const AudioStreamBasicDescription*)inData)->mFormatID !=
                     ...
                     "NullAudio_SetStreamPropertyData: unsupported format ID "
                     "for kAudioStreamPropertyPhysicalFormat");
      FailWithAction(
          ((const AudioStreamBasicDescription*)inData)->mFormatFlags !=
              (kAudioFormatFlagIsFloat | kAudioFormatFlagsNativeEndian |
               kAudioFormatFlagIsPacked),
          theAnswer = kAudioDeviceUnsupportedFormatError, Done,
          "NullAudio_SetStreamPropertyData: unsupported format flags for "
          "kAudioStreamPropertyPhysicalFormat");
      FailWithAction(
          ((const AudioStreamBasicDescription*)inData)->mBytesPerPacket != 8,
          theAnswer = kAudioDeviceUnsupportedFormatError, Done,
          "NullAudio_SetStreamPropertyData: unsupported bytes per packet for "
          "kAudioStreamPropertyPhysicalFormat");
      FailWithAction(
          ((const AudioStreamBasicDescription*)inData)->mFramesPerPacket != 1,
          theAnswer = kAudioDeviceUnsupportedFormatError, Done,
          "NullAudio_SetStreamPropertyData: unsupported frames per packet for "
          "kAudioStreamPropertyPhysicalFormat");
      FailWithAction(
          ((const AudioStreamBasicDescription*)inData)->mBytesPerFrame != 8,
          theAnswer = kAudioDeviceUnsupportedFormatError, Done,
          "NullAudio_SetStreamPropertyData: unsupported bytes per frame for "
          "kAudioStreamPropertyPhysicalFormat");
      FailWithAction(
          ((const AudioStreamBasicDescription*)inData)->mChannelsPerFrame != 2,
          theAnswer = kAudioDeviceUnsupportedFormatError, Done,
          "NullAudio_SetStreamPropertyData: unsupported channels per frame for "
          "kAudioStreamPropertyPhysicalFormat");
      FailWithAction(
          ((const AudioStreamBasicDescription*)inData)->mBitsPerChannel != 32,
          theAnswer = kAudioDeviceUnsupportedFormatError, Done,
          "NullAudio_SetStreamPropertyData: unsupported bits per channel for "
          "kAudioStreamPropertyPhysicalFormat");
      FailWithAction(
          (((const AudioStreamBasicDescription*)inData)->mSampleRate !=
           44100.0) &&
              (((const AudioStreamBasicDescription*)inData)->mSampleRate !=
               48000.0),
          theAnswer = kAudioHardwareIllegalOperationError, Done,
          "NullAudio_SetStreamPropertyData: unsupported sample rate for "
          "kAudioStreamPropertyPhysicalFormat");

      //	If we made it this far, the requested format is something we
      //support, so make sure the sample rate is actually different
      pthread_mutex_lock(&gPlugIn_StateMutex);
      theOldSampleRate = gDevice_SampleRate;
      pthread_mutex_unlock(&gPlugIn_StateMutex);
...
#pragma unused(inClientProcessID)
  ...
  //properties plus a few 	extras that are useful but not required. There is more
  //detailed commentary about each 	property in the
  //NullAudio_GetControlPropertyData() method.
  switch (inObjectID) {
    case kObjectID_Volume_Input_Master:
    case kObjectID_Volume_Output_Master:
      switch (inAddress->mSelector) {
...
#pragma unused(inClientProcessID)
  ...
  FailWithAction(outDataSize == NULL,
                 theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "NullAudio_GetControlPropertyData: no place to put the return "
                 "value size");
  FailWithAction(
      outData == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done,
      ...
      switch (inAddress->mSelector) {
        case kAudioObjectPropertyBaseClass:
          ...
          FailWithAction(
              inDataSize < sizeof(AudioClassID),
              theAnswer = kAudioHardwareBadPropertySizeError, Done,
              "NullAudio_GetControlPropertyData: not enough space for the "
              "return value of kAudioObjectPropertyBaseClass for the volume "
              "control");
        ...
        case kAudioObjectPropertyClass:
          ...
          FailWithAction(inDataSize < sizeof(AudioClassID),
                         theAnswer = kAudioHardwareBadPropertySizeError, Done,
                         "NullAudio_GetControlPropertyData: not enough space "
                         "for the return value of kAudioObjectPropertyClass "
        ...
        case kAudioObjectPropertyOwner:
          ...
          FailWithAction(inDataSize < sizeof(AudioObjectID),
                         theAnswer = kAudioHardwareBadPropertySizeError, Done,
                         "NullAudio_GetControlPropertyData: not enough space "
                         "for the return value of kAudioObjectPropertyOwner "
                         "for the volume control");
          *((AudioObjectID*)outData) = kObjectID_Device;
          *outDataSize = sizeof(AudioObjectID);
          break;
        ...
        case kAudioControlPropertyScope:
          ...
          FailWithAction(inDataSize < sizeof(AudioObjectPropertyScope),
                         theAnswer = kAudioHardwareBadPropertySizeError, Done,
                         "NullAudio_GetControlPropertyData: not enough space "
                         "for the return value of kAudioControlPropertyScope "
                         "for the volume control");
        ...
        case kAudioControlPropertyElement:
          ...
          FailWithAction(inDataSize < sizeof(AudioObjectPropertyElement),
                         theAnswer = kAudioHardwareBadPropertySizeError, Done,
                         "NullAudio_GetControlPropertyData: not enough space "
                         "for the return value of kAudioControlPropertyElement "
                         "for the volume control");
        ...
        case kAudioLevelControlPropertyScalarValue:
          ...
          //0 to 1. 	Note that we need to take the state lock to examine the
          //value.
          FailWithAction(
              inDataSize < sizeof(Float32),
              theAnswer = kAudioHardwareBadPropertySizeError, Done,
              "NullAudio_GetControlPropertyData: not enough space for the "
              "return value of kAudioLevelControlPropertyScalarValue for the "
              "volume control");
          pthread_mutex_lock(&gPlugIn_StateMutex);
        ...
        case kAudioLevelControlPropertyDecibelValue:
          ...
          FailWithAction(
              ...
              theAnswer = kAudioHardwareBadPropertySizeError, Done,
              "NullAudio_GetControlPropertyData: not enough space for the "
              "return value of kAudioLevelControlPropertyDecibelValue for the "
              "volume control");
          pthread_mutex_lock(&gPlugIn_StateMutex);
          *((Float32*)outData) = (inObjectID == kObjectID_Volume_Input_Master)
                                     ? gVolume_Input_Master_Value
        ...
        case kAudioLevelControlPropertyDecibelRange:
          ...
          FailWithAction(
              ...
              theAnswer = kAudioHardwareBadPropertySizeError, Done,
              "NullAudio_GetControlPropertyData: not enough space for the "
              "return value of kAudioLevelControlPropertyDecibelRange for the "
              "volume control");
      ...
      switch (inAddress->mSelector) {
        ...
        case kAudioObjectPropertyClass:
          ...
          FailWithAction(
              ...
              theAnswer = kAudioHardwareBadPropertySizeError, Done,
              "NullAudio_GetControlPropertyData: not enough space for the "
              "return value of kAudioObjectPropertyClass for the mute control");
        ...
        case kAudioObjectPropertyOwner:
          ...
          FailWithAction(
              ...
              theAnswer = kAudioHardwareBadPropertySizeError, Done,
              "NullAudio_GetControlPropertyData: not enough space for the "
              "return value of kAudioObjectPropertyOwner for the mute control");
          *((AudioObjectID*)outData) = kObjectID_Device;
          *outDataSize = sizeof(AudioObjectID);
          break;
        ...
        case kAudioBooleanControlPropertyValue:
          ...
          //mute is off 	and audio can be heard and 1 means that mute is on and
          //audio cannot be heard. 	Note that we need to take the state lock to
          //examine this value.
          FailWithAction(
              inDataSize < sizeof(UInt32),
              theAnswer = kAudioHardwareBadPropertySizeError, Done,
              "NullAudio_GetControlPropertyData: not enough space for the "
              "return value of kAudioBooleanControlPropertyValue for the mute "
              "control");
          pthread_mutex_lock(&gPlugIn_StateMutex);
      ...
      switch (inAddress->mSelector) {
        case kAudioObjectPropertyBaseClass:
          ...
          FailWithAction(
              ...
              theAnswer = kAudioHardwareBadPropertySizeError, Done,
              "NullAudio_GetControlPropertyData: not enough space for the "
              "return value of kAudioObjectPropertyBaseClass for the data "
              "source control");
        ...
        case kAudioSelectorControlPropertyCurrentItem:
          ...
          FailWithAction(
              ...
              theAnswer = kAudioHardwareBadPropertySizeError, Done,
              "NullAudio_GetControlPropertyData: not enough space for the "
              "return value of kAudioSelectorControlPropertyCurrentItem for "
        ...
        case kAudioSelectorControlPropertyAvailableItems:
          ...
          //	Calculate the number of items that have been requested. Note
          //that this 	number is allowed to be smaller than the actual size of
          //the list. In such 	case, only that number of items will be returned
        ...
        case kAudioSelectorControlPropertyItemName:
          ...
          FailWithAction(
              ...
              theAnswer = kAudioHardwareBadPropertySizeError, Done,
              "NullAudio_GetControlPropertyData: not enough space for the "
              "return value of kAudioSelectorControlPropertyItemName for the "
              "data source control");
          FailWithAction(inQualifierDataSize != sizeof(UInt32),
                         theAnswer = kAudioHardwareBadPropertySizeError, Done,
                         "NullAudio_GetControlPropertyData: wrong size for the "
                         "qualifier of kAudioSelectorControlPropertyItemName "
                         "for the data source control");
          FailWithAction(
              *((const UInt32*)inQualifierData) >= kDataSource_NumberItems,
              theAnswer = kAudioHardwareIllegalOperationError, Done,
              "NullAudio_GetControlPropertyData: the item in the qualifier is "
              "not valid for kAudioSelectorControlPropertyItemName for the "
              "data source control");
...
#pragma unused(inClientProcessID, inQualifierDataSize, inQualifierData)
  ...
  FailWithAction(outNumberPropertiesChanged == NULL,
                 theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "NullAudio_SetControlPropertyData: no place to return the "
                 "number of properties that changed");
  FailWithAction(outChangedAddresses == NULL,
                 theAnswer = kAudioHardwareIllegalOperationError, Done,
                 "NullAudio_SetControlPropertyData: no place to return the "
  ...
  *outNumberPropertiesChanged = 0;
  ...
  switch (inObjectID) {
    ...
    case kObjectID_Volume_Output_Master:
      switch (inAddress->mSelector) {
        case kAudioLevelControlPropertyScalarValue:
          ...
          //that if this 	value changes, it implies that the dB value changed
          //too.
          FailWithAction(inDataSize != sizeof(Float32),
                         theAnswer = kAudioHardwareBadPropertySizeError, Done,
                         "NullAudio_SetControlPropertyData: wrong size for the "
        ...
        case kAudioLevelControlPropertyDecibelValue:
          ...
          //that is how 	the value is tracked. Note that if this value changes,
          //it implies that the 	scalar value changes as well.
          FailWithAction(inDataSize != sizeof(Float32),
                         theAnswer = kAudioHardwareBadPropertySizeError, Done,
                         "NullAudio_SetControlPropertyData: wrong size for the "
                         "data for kAudioLevelControlPropertyScalarValue");
    ...
    case kObjectID_DataDestination_PlayThru_Master:
      switch (inAddress->mSelector) {
        case kAudioSelectorControlPropertyCurrentItem:
          //	For selector controls, we check to make sure the requested value
          //is in the 	available items list and just store the value.
          FailWithAction(inDataSize != sizeof(UInt32),
                         theAnswer = kAudioHardwareBadPropertySizeError, Done,
                         "NullAudio_SetControlPropertyData: wrong size for the "
          ...
          FailWithAction(*((const UInt32*)inData) >= kDataSource_NumberItems,
                         theAnswer = kAudioHardwareIllegalOperationError, Done,
                         "NullAudio_SetControlPropertyData: requested item not "
                         "in available items list for "
...
#pragma unused(inClientID)
  ...
  theHostTickOffset =
      ((Float64)(gDevice_NumberTimeStamps + 1)) * theHostTicksPerRingBuffer;
  ...
  *outHostTime = gDevice_AnchorHostTime + (((Float64)gDevice_NumberTimeStamps) *
                                           theHostTicksPerRingBuffer);
  *outSeed = 1;
...
static OSStatus NullAudio_DoIOOperation(
    ...
    void* ioMainBuffer,
    void* ioSecondaryBuffer) {
  //	This is called to actuall perform a given operation. For this device,
  //all we need to do is 	clear the buffer for the ReadInput operation.
